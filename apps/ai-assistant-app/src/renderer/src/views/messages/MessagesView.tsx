...
// appear — the entry animation for a freshly-added bubble: opacity 0→1 over the first 100ms,
// then a quick light flash underneath (box-shadow grows then shrinks) over the next 100ms,
// once the fade has finished. Total 200ms.
const appear = keyframes`
  0% { opacity: 0; box-shadow: 0 0 0 0 rgba(242, 204, 96, 0); }
  50% { opacity: 1; box-shadow: 0 0 0 0 rgba(242, 204, 96, 0); }
  75% { opacity: 1; box-shadow: 0 8px 26px 3px rgba(242, 204, 96, 0.75); }
  100% { opacity: 1; box-shadow: 0 0 0 0 rgba(242, 204, 96, 0); }
`
...
const Bubble = styled.div<{ $role: Role; $error?: boolean }>`
  ...
  /* Assistant (answer) bubbles use the comic palette — solid yellow-orange, dark bold text —
     matching the off-screen comic bubble. User bubbles stay dark; errors stay dark + red. */
  border: 1px solid ${(p) => (p.$role === 'assistant' && !p.$error ? '#4a3300' : colors.border)};
  background: ${(p) => (p.$error ? '#1d2530' : p.$role === 'assistant' ? '#ffc24a' : '#11161d')};
  color: ${(p) => (p.$error ? colors.danger : p.$role === 'assistant' ? '#1a1205' : '#e6edf3')};
  font-weight: ${(p) => (p.$role === 'assistant' && !p.$error ? 600 : 400)};
...
export default function MessagesView({ ctx }: { ctx: ViewContext }): React.JSX.Element {
  ...
  // Wallpaper behind the chat log (dark overlay keeps bubbles readable) — same treatment
  ...
  return (
    <Wrap style={{ backgroundImage, backgroundSize: 'cover', backgroundPosition: 'center' }}>
      {messages.length === 0 ? (
