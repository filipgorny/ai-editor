...
export default function NodeContextMenu({
  ...
  if (isContainer) {
    const items: MenuEntry[] = [
      ...
      { icon: '✏️', label: t('graph.rename'), onClick: () => onRename(node) },
      { icon: '🗑️', label: t('graph.deleteElement'), onClick: () => onDelete(node) },
      { header: t('graph.addElement') },
      { icon: '🟥', label: t('graph.class'), onClick: () => onAdd(addTargetDir(node), 'class', node) },
      { icon: 'λ', label: t('graph.function'), onClick: () => onAdd(addTargetDir(node), 'function', node) },
      { icon: '📁', label: t('graph.folder'), onClick: () => onAdd(addTargetDir(node), 'folder', node) }
    ]
  ...
  const items: MenuEntry[] = [
    ...(node.absFile ? [{ icon: '📝', label: t('graph.edit'), onClick: () => onEdit(node) }] : [])
  ]

  if (isCodeEntity) {
    items.push({ icon: '📋', label: t('graph.editMethods'), onClick: () => onEditMethods(node) })
    items.push({
      icon: '💬',
    ...
    if (implemented) {
      items.push({ icon: '✅', label: t('graph.alreadyImplemented'), onClick: () => undefined })
    } else {
      items.push({ icon: '⚙️', label: t('graph.generateCode'), onClick: () => onGenerate(node) })
    }
  }

  items.push({ icon: '✏️', label: t('graph.rename'), onClick: () => onRename(node) })
  items.push({ icon: '🗑️', label: t('graph.deleteElement'), onClick: () => onDelete(node) })
