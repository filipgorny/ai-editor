...
import { RunAppContext } from '@/views/diagram/RunAppContext'
import { kindColor } from '@/styles/tokens'
import NodeCard from '@/views/diagram/NodeCard'
import TempCard from '@/views/diagram/TempCard'
import NodeContextMenu from '@/views/diagram/NodeContextMenu'

const nodeTypes = { entity: NodeCard, temp: TempCard }

// Container kinds that expand/collapse their children in place (and drill in on double-click).
// PackageNode extends AppNode, so 'package' behaves exactly like 'app' (it carries an appId
// and loads its internals on first expand).
const isExpandableKind = (k: string): boolean => k === 'folder' || k === 'app' || k === 'package'
const isAppLikeKind = (k: string): boolean => k === 'app' || k === 'package'
...
function estHeight(n: Node | undefined): number {
  ...
  // Footer (language left / framework right) is shown for app/package nodes and for anything
  // carrying language or framework info — reserve a row so cards don't overlap in the tree.
  const hasFooter = !!n.framework || !!n.language || n.kind === 'app' || n.kind === 'package'
  const footerH = hasFooter ? 30 : 0
...
export default function GraphView({
  ...
  const { rfNodes, rfEdges } = useMemo(() => {
    ...
    const reveal = (id: string): void => {
      ...
      // folders AND apps/packages reveal their children when expanded (expand inline)
      if (n && isExpandableKind(n.kind) && expanded.has(id)) {
  ...
  // ReactFlow owns the live node objects (useNodesState) so it keeps each node's MEASURED
  // width/height across renders. Rebuilding the nodes array from scratch every render (the old
  // controlled pattern) made a dragged node lose its measured size — ReactFlow then renders an
  // unmeasured node as visibility:hidden, so it "vanished for good" until a re-scan. We seed
  // from the derived layout (mergedNodes) and re-sync whenever it changes, carrying over the
  // measured dimensions ReactFlow already has so re-synced nodes stay visible.
  const [nodes, setNodes, onNodesChangeRaw] = useNodesState(mergedNodes)

  useEffect(() => {
    setNodes((cur) => {
      const dim = new Map(cur.map((n) => [n.id, { width: n.width, height: n.height }]))

      return mergedNodes.map((n) => {
        const d = dim.get(n.id)

        return d && d.width != null ? { ...n, width: d.width, height: d.height } : n
      })
    })
  }, [mergedNodes, setNodes])

  // Selection is driven externally (selectedId → mergedNodes), so drop ReactFlow's own 'select'
  // changes to avoid two sources of truth. Position AND dimensions changes pass through — the
  // dimensions ones are exactly what keep a dragged node measured and visible.
  const onNodesChange = useCallback(
    (changes: NodeChange[]): void => onNodesChangeRaw(changes.filter((c) => c.type !== 'select')),
    [onNodesChangeRaw]
  )
  ...
  // Po rozwinięciu KONTENERA (aplikacji lub folderu) wjedź kamerą w JEGO dzieci (drill-in) —
  // dopasuj widok do poddrzewa, a nie do całego projektu. Dla aplikacji czeka, aż wewnętrzne
  // węzły się doładują (skan jest asynchroniczny); dla folderu dzieci są już w grafie.
  useEffect(() => {
    ...
    // Zbierz WIDOCZNE poddrzewo aplikacji (app + rozwinięte potomstwo) i dopasuj kamerę
    // tylko do nich — dzięki temu po otwarciu aplikacji widać jej dzieci, a nie cały projekt.
    const childrenOf = new Map<string, string[]>()

    for (const d of graph.dependencies()) {
      if (d.kind === 'contains') {
        childrenOf.set(d.from, [...(childrenOf.get(d.from) ?? []), d.to])
      }
    }
    ...
    const t = window.setTimeout(() => flow.current?.fitView({ nodes: subtree, padding: 0.3, duration: 400 }), 80)
  ...
  const toggleExpand = (node: Node) => {
    ...
    if (willExpand) {
      if (isAppLikeKind(node.kind) && kids.length === 0 && node instanceof AppNode) {
        onExpandApp?.(node.appId) // not loaded yet → fetch + merge its internals
      }

      // Expanding a container (app OR folder): drill the camera into its children — fit the
      // node's visible subtree so EVERYTHING it contains is on screen (e.g. all apps inside
      // the `apps` folder), instead of zooming out to the whole project or panning to one child.
  ...
  const handleDoubleClick = (node: Node) => {
    ...
    // Folders and apps/packages expand/collapse in place. An app/package loads its internal
    // graph (onExpandApp) the first time it's expanded — the monorepo stays visible.
    if (isExpandableKind(node.kind)) {
      toggleExpand(node)
  ...
  const resolveNodeMeta = (node: Node) => {
    if (isExpandableKind(node.kind) || !node.absFile) {
      return
    }
  ...
  return (
    ...
    <ReactFlow
      ...
      onNodeDragStop={(_e, _node, dragged) => {
        // Persist the dropped position so it survives the next layout re-sync (expand, select,
        // refresh). ReactFlow already moved the node live; we only record the final spot.
        setPosOverride((prev) => {
          const next = { ...prev }
