import { _ as __vitePreload, e as commonjsGlobal } from "./index-CWiMyzFO.js";
...
(function(module, exports) {
  (function(global, factory) {
    ...
    class Thread {
      ...
      setTop(index2) {
        this.lua.lua_settop(this.address, index2);
      }
      remove(index2) {
        return this.lua.lua_remove(this.address, index2);
      }
      setField(index2, name, value) {
        index2 = this.lua.lua_absindex(this.address, index2);
        this.pushValue(value);
        this.lua.lua_setfield(this.address, index2, name);
      }
      ...
      setMetatable(index2, metatable) {
        index2 = this.lua.lua_absindex(this.address, index2);
        if (this.lua.lua_getmetatable(this.address, index2)) {
          this.pop(1);
          const name = this.getMetatableName(index2);
          throw new Error(`data already has associated metatable: ${name || "unknown name"}`);
        }
        this.pushValue(metatable);
        this.lua.lua_setmetatable(this.address, index2);
      }
      getMetatableName(index2) {
        const metatableNameType = this.lua.luaL_getmetafield(this.address, index2, "__name");
      ...
      getValue(index2, inputType, userdata) {
        index2 = this.lua.lua_absindex(this.address, index2);
        const type = inputType !== null && inputType !== void 0 ? inputType : this.lua.lua_type(this.address, index2);
        switch (type) {
          ...
          case exports2.LuaType.Number:
            return this.lua.lua_tonumberx(this.address, index2, null);
          case exports2.LuaType.String:
            return this.lua.lua_tolstring(this.address, index2, null);
          case exports2.LuaType.Boolean:
            return Boolean(this.lua.lua_toboolean(this.address, index2));
          case exports2.LuaType.Thread:
            return this.stateToThread(this.lua.lua_tothread(this.address, index2));
          default: {
            let metatableName;
            if (type === exports2.LuaType.Table || type === exports2.LuaType.Userdata) {
              metatableName = this.getMetatableName(index2);
            }
            const typeExtensionWrapper = this.typeExtensions.find((wrapper) => wrapper.extension.isType(this, index2, type, metatableName));
            if (typeExtensionWrapper) {
              return typeExtensionWrapper.extension.getValue(this, index2, userdata);
            }
            console.warn(`The type '${this.lua.lua_typename(this.address, type)}' returned is not supported on JS`);
            return new Pointer(this.lua.lua_topointer(this.address, index2));
          }
        }
      }
      close() {
      ...
      indexToString(index2) {
        const str = this.lua.luaL_tolstring(this.address, index2, null);
        this.pop();
        return str;
      }
...
${traceback}`;
    ...
    class LuaTypeExtension {
      ...
      getValue(thread, index2, _userdata) {
        const refUserdata = thread.lua.luaL_testudata(thread.address, index2, this.name);
        if (!refUserdata) {
    ...
    class FunctionTypeExtension extends LuaTypeExtension {
      ...
      getValue(thread, index2) {
        var _a;
        thread.lua.lua_pushvalue(thread.address, index2);
    ...
    class ProxyTypeExtension extends LuaTypeExtension {
      ...
      getValue(thread, index2) {
        const refUserdata = thread.lua.lua_touserdata(thread.address, index2);
        const referencePointer = thread.lua.module.getValue(refUserdata, "*");
        return thread.lua.getRef(referencePointer);
      }
    ...
    class TableTypeExtension extends LuaTypeExtension {
      ...
      getValue(thread, index2, userdata) {
        const seenMap = userdata || /* @__PURE__ */ new Map();
        const pointer = thread.lua.lua_topointer(thread.address, index2);
        let table = seenMap.get(pointer);
        if (!table) {
          const keys = this.readTableKeys(thread, index2);
          const isSequential = keys.length > 0 && keys.every((key, index3) => key === String(index3 + 1));
          table = isSequential ? [] : {};
          seenMap.set(pointer, table);
          this.readTableValues(thread, index2, seenMap, table);
        }
        return table;
      }
      ...
      readTableKeys(thread, index2) {
        const keys = [];
        thread.lua.lua_pushnil(thread.address);
        while (thread.lua.lua_next(thread.address, index2)) {
      ...
      readTableValues(thread, index2, seenMap, table) {
    ...
    var initWasmModule = (() => {
      var _scriptDir = typeof document === "undefined" && typeof location === "undefined" ? commonjsRequire("url").pathToFileURL(__filename).href : typeof document === "undefined" ? location.href : _documentCurrentScript && _documentCurrentScript.src || new URL("index.js", document.baseURI).href;
      return async function(moduleArg = {}) {
        ...
        if (n) {
          ...
          const { createRequire: a } = await __vitePreload(async () => {
            const { createRequire: a2 } = await Promise.resolve().then(() => __viteBrowserExternal);
            return { createRequire: a2 };
          }, true ? void 0 : void 0, import.meta.url);
          var require$1 = a(typeof document === "undefined" && typeof location === "undefined" ? commonjsRequire("url").pathToFileURL(__filename).href : typeof document === "undefined" ? location.href : _documentCurrentScript && _documentCurrentScript.src || new URL("index.js", document.baseURI).href), fs = require$1("fs"), na = require$1("path");
          k ? r = na.dirname(r) + "/" : r = require$1("url").fileURLToPath(new URL("./", typeof document === "undefined" && typeof location === "undefined" ? commonjsRequire("url").pathToFileURL(__filename).href : typeof document === "undefined" ? location.href : _documentCurrentScript && _documentCurrentScript.src || new URL("index.js", document.baseURI).href));
          ia = (b, c) => {
        ...
        } else M = new URL("glue.wasm", typeof document === "undefined" && typeof location === "undefined" ? commonjsRequire("url").pathToFileURL(__filename).href : typeof document === "undefined" ? location.href : _documentCurrentScript && _documentCurrentScript.src || new URL("index.js", document.baseURI).href).href;
        function Qa(a) {
          if (ka) return ka(a);
    ...
    class LuaWasm {
      ...
      lua_remove(luaState, index2) {
        this.lua_rotate(luaState, index2, -1);
      ...
      lua_upvalueindex(index2) {
        return LUA_REGISTRYINDEX - index2;
      }
      ref(data) {
        ...
        const index2 = availableIndex === void 0 ? this.referenceMap.size + 1 : availableIndex;
        this.referenceMap.set(index2, data);
        this.referenceTracker.set(data, {
          refCount: 1,
          index: index2
        });
        this.lastRefIndex = index2;
        return index2;
      }
      unref(index2) {
        const ref = this.referenceMap.get(index2);
        if (ref === void 0) {
          return;
        }
        ...
        if (metadata === void 0) {
          ...
          this.availableReferences.push(index2);
          return;
        }
        metadata.refCount--;
        if (metadata.refCount <= 0) {
          this.referenceTracker.delete(ref);
          this.referenceMap.delete(index2);
          this.availableReferences.push(index2);
        }
      }
      getRef(index2) {
        return this.referenceMap.get(index2);
      }
      getLastRefIndex() {
